...
from SelfDrivingCar.deep_q_learning import Dqn
...
brain = Dqn(5,3,0.9) # 4 inputs, 3 actions, gamma = 0.9
...
class Game(Widget):
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        state = [orientation, self.car.signal1, self.car.signal2, self.car.signal3, -orientation] # our input state vector, composed of the orientation plus the three signals received by the three sensors
